...
def preprocess_text_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[config.TICKET_SUMMARY] = df[config.TICKET_SUMMARY].astype(str).fillna('')
    df[config.INTERACTION_CONTENT] = df[config.INTERACTION_CONTENT].astype(str).fillna('')
    df[config.TEXT_COL] = df[config.TICKET_SUMMARY].str.lower() + ' ' + df[config.INTERACTION_CONTENT].str.lower()
